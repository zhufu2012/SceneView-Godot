...
public delegate void SceneDataChangedEventHandler(object sender, EventArgs e);
public event SceneDataChangedEventHandler SceneDataChanged;
...
namespace SceneCore_Space
{
    ...
    public class SceneLable
    {
		...
		//当对象发生变化时，运行这个函数
		//sceneLable.SceneDataChanged += (sender, e) =>
		//{
		// 在这里编写当数据变化时需要执行的代码
		//	Console.WriteLine("Scene data has changed!");
		// 可以调用任何需要执行的函数
		//};
		public void OnSceneDataChanged()
		{
			SceneDataChanged?.Invoke(this, EventArgs.Empty);
		}
